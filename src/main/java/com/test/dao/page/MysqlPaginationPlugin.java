...
public class MysqlPaginationPlugin extends BasePaginationPlugin {
	...
	public String getCountSql(String sql) {
		if (sql.indexOf("page_from") > 0) {
			return "select count(1) from " + sql.substring(sql.indexOf("page_from") + 9,
					sql.indexOf("page_order") > 0 ? sql.indexOf("page_order") : sql.length());
		} else if (sql.indexOf("page_order") > 0) {
			return "select count(1) from ( "
					+ sql.substring(0, sql.indexOf("page_order") > 0 ? sql.indexOf("page_order") : sql.length())
					+ ") as temp_ ";
		} else {
